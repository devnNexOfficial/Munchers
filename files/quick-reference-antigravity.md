# quick-reference-antigravity.md — Muncherz
# Quick reference for Antigravity AI code editor.
# Full detail lives in ai-instructions.md — READ THAT FIRST, every time.
# Antigravity does not auto-detect this file — paste/reference it manually
...
## How to prompt Antigravity with this file
Since Antigravity doesn't auto-load project rule files the way Cursor's
`.cursorrules` does, structure every prompt like this:
...
4. Ask Antigravity to confirm it understood the core/flexible ingredient
   rule and the server-side price validation rule before generating code,
   if the task touches the customizer or order placement
```

## Workflow
One checklist item (function/component level) at a time. Reference ai-instructions.md content in every prompt — Antigravity will not read it automatically from disk unless explicitly given the content or file path in-context. Run type-check + lint + build before commit.
