...
✅ Kitchen LCD PIN Security
   - PIN hashed before storage (never plaintext) — use `bcryptjs` (pure
     JavaScript, no native bindings) or the Web Crypto API's
     `crypto.subtle.digest` with a salt, NOT native `bcrypt`/`argon2`
     packages. Native bcrypt/argon2 bindings are C++ compiled modules
     that frequently fail to build or fail at runtime on Vercel
     Serverless Functions and are not supported at all on Edge Runtime
     — this would surface as a deployment crash, not a code review
     catch, if used here.
